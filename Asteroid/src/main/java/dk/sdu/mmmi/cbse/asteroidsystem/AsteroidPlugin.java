...
public class AsteroidPlugin implements IEventListener {
	...
	private Entity createNewAsteroid(GameData gameData) {
		...
		asteroid.setPolygonCoordinates(-8, 0, -6, 3, -4, 4.75, -1.25, 4.6, 0.5, 2, 4, 1.3, 4, -1.5, 2, -2.78, 4, -4, 2,
				-5.25, 0, -6, -4, -6, -6, -3);
	...
	public void onTrigger(Event event) {
		for (Asteroid asteroid : asteroids) {
			if (event.getEntities()[0] == asteroid && event.getEntities()[1] instanceof Bullet) {
				...
				Event newEvent = new Event(EventType.ASTEROID_DESTROYED, event.getWorld(), event.getGameData(),
						asteroid);
