	...
	public void process(double deltaTime, GameData gameData, World world) {
		...
		for (Entity entity : world.getEntities(Asteroid.class)) {
			if (entity instanceof Asteroid asteroid) {
				double speed = 0.6;
				asteroid.setX(
						(asteroid.getX() + asteroid.getXDirection() * deltaTime * speed) % gameData.getDisplayWidth());
				asteroid.setY(
						(asteroid.getY() + asteroid.getYDirection() * deltaTime * speed) % gameData.getDisplayHeight());
